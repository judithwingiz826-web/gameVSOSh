...
function checkAnswer() {
  ...
  const answerRaw = el.answer.value.trim();
  if (!answerRaw) {
    el.feedback.textContent = 'Введите ответ перед проверкой.';
    return;
  }

  state.attempts += 1;
  const ok = isCorrectAnswer(state.current, answerRaw);
  if (ok) {
    state.solved += 1;
...
if (!Object.values(el).every(Boolean)) {
  throw new Error('UI elements are missing. Check index.html IDs.');
}
